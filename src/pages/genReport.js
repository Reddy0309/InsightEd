...
const sendReportToParent = async (usn, name, headers, studentDetails) => {
  ...
  console.log(studentDetails[1])
     const response = await axios.post('http://localhost:5000/api/msg/send', { sender: user.name, receiver:studentDetails[1],message: link.href });
    ...
    <div>
      ...
      <div className="signuP-PagE"> 
        <div className="signuP-ContaineR"> 
          ...
          <p className='p'><b>Scroll down to Generate and Download Reports in PDf format and send the reports to the respective parents</b></p>
          {studentsData.map((student, index) => (
            <div key={index} style={{ marginBottom: '20px' }}>
              <p>{`Student ${index + 1}: ${student.slice(1).join(', ')}`}</p>
              <button onClick={() => generatePDF(headers, student)} className="signuP-ButtoN gen">Generate PDF</button>
              <button onClick={() => sendReportToParent(student[0], student[1], headers, student)} className="signuP-ButtoN rep">Send Report to Parent</button> 
            </div>
          ))}
        </div>
      </div>
