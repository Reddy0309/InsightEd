...
const navigate = useNavigate();
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.post('http://localhost:5000/api/msg/parentsee', {
  ...
  return (
    <div className="parent-dashboard-page">
            ...
            <div className="dashboard-container">
                <h2 className="dashboard-welcome mEntor" >Messages from the Mentor</h2>
                <div className="button-row">
                    <button className="action-button BACK" onClick={handleGoBack}>Go Back</button>
                </div>
                <div style={{ maxHeight: '400px', overflowY: 'auto', textAlign: 'left', maxWidth: '600px', margin: '0 auto' }}>
                    ...
                    {messages.map((msg, index) => (
                        <div key={index} style={{ marginBottom: '8px', padding: '8px', border: '1px solid #ccc', borderRadius: '4px', maxHeight: '200px', overflowY: 'auto' }}>
                            {index + 1}. {typeof msg.message === "string" && msg.message.startsWith("blob") ? (
                                <a href={msg.message} download={`report_${index + 1}.pdf`}>Download PDF</a>
                            ) : (
                                msg.message
                            )}
                            <br />
