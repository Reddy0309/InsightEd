...
const Talk = () => {
  ...
  const sendMessage = async (e) => {
    ...
    if (message.trim()) {
      try {
        const response = await axios.post('http://localhost:5000/api/msg/send', {
          sender: user.USN,
      ...
      <div className="dashboard-container">
        ...
        <form onSubmit={sendMessage} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', marginTop: '10px' }}>
          <input
            ...
            className='texttalk'
          />
          <button type="submit" style={{ marginLeft: '10px', padding: '8px', borderRadius: '5px', border: 'none', backgroundColor: '#5D22DC', color: '#fff' }}>Send</button>
        </form>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', marginTop: '10px' }}>
          <div>
            <label htmlFor="language" className='Select'>Select Language:</label>
