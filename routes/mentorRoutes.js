  ...
  router.post('/update-sheet', upload.single('file'), async (req, res) => {
    try {
      ...
      const worksheet = workbook.Sheets[sheetName];
       ...
       res.status(500).json({ message: 'Server error while updating Excel sheet' });
     }
   });
...
router.post('/update-sheet', upload.single('file'), async (req, res) => {
  try {
    ...
    const worksheet = workbook.Sheets[sheetName];
     ...
     // Write the updated workbook back to the file path
     xlsx.writeFile(workbook, "C:/Users/rteja/OneDrive/Desktop/6/MINIPROJECT/6Buploadmarkssheet.xlsx");
