...
router.post('/see', async (req, res) => {
  ...
  const messages = await Message.find({ sender:usn, receiver: name }).sort({ timestamp: -1 });
  res.json(messages);
});

router.post('/parentsee', async (req, res) => {
  const { name, usn } = req.body;
  const messages = await Message.find({ sender:name, receiver: usn }).sort({ timestamp: -1 });
  res.json(messages);
});
...
router.post('/send-notifications', async (req, res) => {
  ...
  try {
      for (const student of students) {
          ...
          console.log(`Sending PTM notification to parent with USN ${usn}: ${messageText}`);
      }
...
router.get('/notifications/:usn', async (req, res) => {
  ...
  try {
      const notifications = await Message.find({ receiver: usn, sender: 'Mentor' }).sort({ timestamp: -1 });
