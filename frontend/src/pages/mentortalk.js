...
const MentorTalk = () => {
    ...
    if (message.trim()) {
      try {
        const response = await axios.post("/api/msg/send", {
          sender: user["NAME OF THE MENTOR"],
          receiver: user.USN,
          message: message,
        });
  ...
  return (
    <div className="parent-dashboard-page">
      {" "}
      {/* Apply parent-dashboard-page class */}
      <Header showBackButton={true} hideButtons={true} />{" "}
      {/* Include Header component */}
      <div className="dashboard-container">
        {" "}
        {/* Apply dashboard-container class */}
        <h2 className="dashboard-welcome">Chat with Parent</h2>{" "}
        {/* Apply dashboard-welcome class */}
        <div className="button-row">
          {" "}
          {/* Apply button-row class */}
